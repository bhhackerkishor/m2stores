...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await CustomerAnalyticsService.overview(range as any, from, to);
    ...
    logger.error("Customer analytics error", "analytics", { error: String(error) });
    return NextResponse.json(errorResponse("INTERNAL_ERROR", "Failed to load customer analytics"), {
      status: 500,
    });
  }
}

...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await RevenueAnalyticsService.overview(range as any, from, to);
    ...
    logger.error("Revenue analytics error", "analytics", { error: String(error) });
    return NextResponse.json(errorResponse("INTERNAL_ERROR", "Failed to load revenue analytics"), {
      status: 500,
    });
  }
}

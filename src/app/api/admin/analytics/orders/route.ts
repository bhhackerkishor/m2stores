...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await OrderAnalyticsService.overview(range as any, from, to);
    ...
    logger.error("Order analytics error", "analytics", { error: String(error) });
    return NextResponse.json(errorResponse("INTERNAL_ERROR", "Failed to load order analytics"), {
      status: 500,
    });
  }
}

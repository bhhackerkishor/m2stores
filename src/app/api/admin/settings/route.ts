...
const schema = z.object({
  ...
  codPincodeMode: z.enum(["all", "allowlist", "blocklist"]).optional(),
  codAllowedPincodes: z.array(z.string().regex(/^\d{6}$/)).max(2000).optional(),
  codAllowedPrefixes: z.array(z.string().regex(/^\d{3}$/)).max(200).optional(),
  codBlockedPincodes: z.array(z.string().regex(/^\d{6}$/)).max(2000).optional(),
  deliveryPincodeMode: z.enum(["all", "allowlist", "blocklist"]).optional(),
  deliveryAllowedPincodes: z.array(z.string().regex(/^\d{6}$/)).max(2000).optional(),
  deliveryAllowedPrefixes: z.array(z.string().regex(/^\d{3}$/)).max(200).optional(),
  deliveryBlockedPincodes: z.array(z.string().regex(/^\d{6}$/)).max(2000).optional(),

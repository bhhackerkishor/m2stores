// components/storefront/ProductDetailPage.tsx
"use client";
...
export function ProductDetailPage({ product  }: ProductPageProps) {
  ...
  const resolvedSku =
    selectedSku ||
    (product.hasVariants ? product.variants[0]?.sku : product.baseSKU) ||
    product._id;

  const wished = isWished(resolvedSku);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      {/* Breadcrumb */}
      <nav
        className="flex items-center gap-1.5 text-xs text-surface-500 dark:text-surface-400 mb-6 overflow-x-auto scrollbar-none"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-14">
        ...
        <div className="pb-28 lg:pb-0">
          ...
          {/* Title */}
          <h1 className="text-balance mb-3 leading-[1.15]">{product.name}</h1>
          <p className="text-surface-600 dark:text-surface-400 text-sm leading-relaxed mb-5">
            {product.shortDescription}
          </p>

          {/* Price */}
          <div className="flex items-baseline gap-3 mb-5">
            <span className="price-lg">{formatPrice(currentPrice)}</span>
            {product.salePrice && product.basePrice && (
              <span className="text-base text-surface-400 dark:text-surface-500 line-through">
                {formatPrice(product.basePrice)}
              </span>
            )}
            {discount > 0 && <Badge variant="success" size="md">Save {discount}%</Badge>}
          </div>
            ...
            <div className="flex items-center gap-2 mb-6">
              <div className="flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg
                    key={i}
                    className={`w-4 h-4 ${i < Math.round(product.averageRating) ? "text-warning-400" : "text-surface-200 dark:text-surface-700"}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
          ...
          {product.hasVariants && (
            <div className="mb-6">
              <h2 className="!text-sm !font-semibold font-sans text-surface-900 dark:text-surface-100 mb-3">
                Select Options
              </h2>
              <VariantSelector variants={product.variants} variantNames={variantNames} onSelect={setSelectedSku} />
            </div>
          )}

          {/* Quantity */}
          <div className="flex items-center gap-3 mb-6">
            <label className="text-sm font-semibold text-surface-900 dark:text-surface-100">Quantity</label>
            <div className="flex items-center border border-surface-200 dark:border-surface-700 rounded-sm overflow-hidden">
              <button
              ...
              <span className="w-12 text-center font-semibold tabular-nums price-sm !text-surface-900 dark:!text-surface-100">
                {quantity}
              </span>
              <button
          ...
          {/* Notice */}
          {notice && (
            <div className="mb-4 p-3 bg-warning-50 dark:bg-warning-950/30 border border-warning-200 dark:border-warning-800 rounded-sm text-warning-700 dark:text-warning-400 text-sm flex items-center gap-2">
          ...
          <div className="hidden lg:block">
            ...
            <Button onClick={() => handleAddToCart(true)} isLoading={isAdding} variant="outline" className="w-full mb-8" size="lg">
              Buy Now
            </Button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-8">
            {[
              ...
              <div key={f.label} className="flex flex-col items-center p-3 bg-surface-50 dark:bg-surface-800 rounded-lg text-center">
          ...
          {product.specifications.length > 0 && (
            <div className="border-t border-surface-200 dark:border-surface-800 pt-6 mt-6">
              <h2 className="!text-sm !font-semibold font-sans text-surface-900 dark:text-surface-100 mb-3">
                Specifications
              </h2>
      ...
      <div className="lg:hidden fixed bottom-[60px] left-0 right-0 z-30 bg-white/95 dark:bg-surface-950/95 backdrop-blur-md border-t border-surface-200 dark:border-surface-800 px-4 py-3 flex items-center gap-3 pb-safe">
        <div className="flex-1 min-w-0">
          ...
          <p className="price-md">{formatPrice(currentPrice)}</p>
        </div>
        <button
          onClick={handleWishlist}
          aria-label={wished ? "Remove from wishlist" : "Add to wishlist"}
          className="btn btn-icon border border-surface-200 dark:border-surface-700 shrink-0"
        >
          <Heart className={`w-5 h-5 ${wished ? "fill-danger-500 text-danger-500" : "text-surface-600"}`} />
        </button>

...
export function ReturnSection({ orderNumber, eligible, eligibilityReason, eligibleItems, existing, windowDays }: Props) {
  ...
  return (
    <Card>
      ...
      <p className="text-xs text-surface-500 mb-4">{windowDays}-day return window on delivered orders. Partial returns supported.</p>

      {existing.length > 0 && (
        <div className="space-y-2 mb-4">
          {existing.map((r) => (
            <div key={r._id} className="p-3 bg-surface-50 rounded-lg text-sm">
              <div className="flex justify-between flex-wrap gap-1">
                <span className="font-mono font-bold">{r.rmaNumber}</span>
                <span className="px-2 py-0.5 bg-white border rounded-full text-xs font-semibold">{r.status} · refund {r.refund.status}</span>
              </div>
      ...
      {!eligible ? (
        <p className="text-sm text-surface-500">{eligibilityReason || "Returns are not available for this order right now."}</p>
      ) : !show ? (
        <Button variant="outline" onClick={() => setShow(true)}>Request Return</Button>
      ) : (
        <div className="space-y-3">
          {eligibleItems.filter((i) => i.returnableQty > 0).map((i) => (
            <div key={i.sku} className="flex items-center justify-between gap-2 text-sm">
              <div className="min-w-0">
                <p className="font-medium truncate">{i.name}</p>
                <p className="text-xs text-surface-500 font-mono">{i.sku} · max {i.returnableQty}</p>
              </div>
              <Input
                type="number"
                min={0}
          ...
          <div className="flex gap-2">
            <Button onClick={submit} isLoading={busy} size="sm">Submit Request</Button>
